...
function renderMenu(records, selected, selectedSids, codexRoot, status = "") {
  ...
  for (let idx = start; idx < end; idx += 1) {
    ...
    const mark = selectedSids.has(record.sid) ? "x" : " ";
    let line = `[${mark}] [${String(idx + 1).padStart(seqWidth, "0")}] ${fit(displayLabel(record, labelWidth), labelWidth)} | ${shortId(record.sid)} | 状:${recordStateShort(record)} 线:${record.threadExists ? "有" : "无"} 历:${record.historyCount} 日:${record.logCount} 回:${record.rolloutCount}`;
  ...
  try {
    while (true) {
      ...
      if (key === "\x1b[A" || key === "k" || key === "K") {
        ...
        const sid = records[selected].sid;
        ...
        if (await promptConfirm(selectedRecords, recursiveSids.length)) {
          const deleted = await purgeSessions(codexRoot, recursiveSids);
          console.log("\nDeleted:");
          for (const record of selectedRecords) {
            console.log(`  session_id: ${record.sid}`);
            console.log(`  label: ${displayLabel(record)}`);
            if (record.firstQuestionText) console.log(`  first_question: ${compact(record.firstQuestionText, 80)}`);
            if (record.firstAnswerText) console.log(`  first_answer: ${compact(record.firstAnswerText, 80)}`);
            for (const rolloutPath of record.rolloutPaths) console.log(`  rollout_jsonl: ${rolloutPath}`);
          }
          console.log(`  recursive_session_count: ${recursiveSids.length}`);
